...
def _get_inference_fn(model_fns, features):
    ...
    def inference_fn(inputs, state, time=None):
        local_features = {
        ...
        for (model_fn, model_state) in zip(model_fns, state):
            if model_state:
                logits, new_state = model_fn(local_features, model_state, time=time)
...
def _beam_search_step(time, func, state, batch_size, beam_size, alpha, pad_id,
    ...
    step_log_probs, next_state = func(flat_seqs, flat_state, time=time)
...
def sample(models, features, params):
    ...
    for time in range(max_step):
        ...
        step_log_probs, next_state = decoding_fn(seqs, states, time=time)
